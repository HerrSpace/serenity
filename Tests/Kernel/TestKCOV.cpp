...
// Fork. In the parent: wait for the child to exit and return true. In the child immediately return
// false after the fork, which can then run the actual test. Only one thread per process can open
// the KCOV device at the same time. LibTest runs the tests multithreaded.
void fork_and_kill_parent();
void fork_and_kill_parent() {
    int pid = fork();
    EXPECT(pid >= 0);
    if (pid > 0) { // parent
